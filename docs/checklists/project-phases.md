...
- [x] _(pendiente de documentar)_ ¿Qué es una solución .NET y para qué sirve?
- [x] _(pendiente de documentar)_ Estructura de un proyecto ASP.NET Core Web API
- [x] _(pendiente de documentar)_ ¿Qué hace cada archivo del template?
- [x] _(pendiente de documentar)_ ¿Por qué separar tests del código de producción?
- [x] _(pendiente de documentar)_ ¿Qué es WebApplicationFactory?
- [x] _(pendiente de documentar)_ Ciclo TDD: rojo → verde → refactor
...
## Fase 2 — Feature Tasks: modelo de dominio y GET /tasks

- [ ] **2.1** Crear `TaskItem` (modelo de dominio con Id, Title, IsCompleted)
- [ ] **2.2** Crear interfaz `ITaskRepository` (contrato para el repositorio)
- [ ] **2.3** Implementar `InMemoryTaskRepository` con datos semilla
- [ ] **2.4** Crear `TaskResponse` (DTO de salida)
- [ ] **2.5** Crear `TaskService` con `GetAllAsync`
- [ ] **2.6** Agregar `GetByIdAsync` al servicio
- [ ] **2.7** Crear `TasksController` con `GET /api/tasks`
- [ ] **2.8** Agregar `GET /api/tasks/{id}` al controller
- [ ] **2.9** Registrar dependencias en DI (Program.cs)
- [ ] **2.10** **Prueba**: GET /tasks → 200 + lista JSON
- [ ] **2.11** **Prueba**: GET /tasks/{id} existente → 200 + tarea
- [ ] **2.12** **Prueba**: GET /tasks/{id} inexistente → 404
- [ ] **2.13** Documentar conceptos en `docs/learning/`
- [ ] **2.14** Actualizar `docs/progress/` y `docs/project-state.md`

### Conceptos a aprender

- Modelo de dominio vs DTO
- ¿Qué es un contrato/interfaz y por qué desacoplar?
- Inyección de dependencias (motivación y registro)
- DTO de salida vs entrada
...
|---|---|---|
| 0 — Fundaciones | 5 | ✅ |
| 1 — Scaffolding | 10 + docs | 🟢 En progreso (6/10 + test OK) |
| 2 — GET /tasks | 14 | ⏳ |
| 3 — POST /tasks | 10 | ⏳ |
| 4 — PUT /tasks | 10 | ⏳ |
| 5 — DELETE /tasks | 8 | ⏳ |
